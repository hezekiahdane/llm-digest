...
Key changes: `pages` is optional, `stateSimulators` and `assets` removed, `useSimulatedState` exported.
...
```bash
git add src/components/dev/DevPanelProvider.tsx src/components/dev/useSimulatedState.ts src/components/dev/__tests__/useSimulatedState.test.ts
...
import { defaultDevPanelState } from '@/components/dev/DevPanelProvider';
